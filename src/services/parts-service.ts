...
const createPart =
  (partsData: PartsData) =>
  async (name: string, price: number, carSegmentId: number) => {
    ...
    const part = await partsData.createPart(
      name,
      +carSegmentId,
      +price,
    );
...
const getPartById =
  (partsData: PartsData) => async (partId: number) => {
...
const updatePart =
  (partsData: PartsData) =>
  async (updatedPartData: UpdatePartsData, partId: number) => {
